...
Two ways to use it. A single comparison, all in one go:

    python run_benchmark.py --audio recordings/ --truth labels/ --models whisper,seamless

Or the planned campaign, which is what a Kaggle session wants -- every
configuration written down first, then worked through a run at a time, each one
writing its own CSV before the next begins:

    python run_benchmark.py plan   --audio recordings/ --truth labels/ --out results/
...
`work` stops cleanly when the budget runs out and picks up where it left off
next time, because a finished run is one whose CSV exists. Stop a session
whenever you like; nothing in flight is lost, because nothing is left in
flight.
...
def main(argv=None):
    # A bare subcommand name routes to the campaign; anything else is the
    # one-shot comparison, so the original invocation still works unchanged.
    argv = list(sys.argv[1:] if argv is None else argv)
    if argv and argv[0] in campaign.COMMANDS:
        return _campaign(argv[0], argv[1:])
    return _one_shot(argv)


def _campaign(command, argv):
    parser = argparse.ArgumentParser(prog=f"run_benchmark.py {command}",
                                     description=campaign.COMMANDS[command].__doc__)
    parser.add_argument("--labels", help="a dataset folder's labels.csv")
    parser.add_argument("--manifest")
    parser.add_argument("--audio")
    parser.add_argument("--truth")
    parser.add_argument("--out", default=str(settings.OUT_DIR))
    parser.add_argument("--dry-run", action="store_true",
                        help="stub model, no weights — checks the plumbing in seconds")
    campaign.add_arguments(parser)
    return campaign.COMMANDS[command](parser.parse_args(argv))
...
def _one_shot(argv):
    ...
    parser.add_argument("--list-tiers", action="store_true",
                        help="print which models this hardware can hold, and how")
    parser.add_argument("--dry-run", action="store_true",
    ...
    if args.list_tiers:
        ...
        if not cards:
            usable, cards = 14.8, 2
            print(f"no GPU visible — showing {cards} x {usable} GB (a Kaggle T4 pair)")
        else:
            print(f"{cards} GPU(s), ~{usable:.1f} GB usable each")
        for placement in tiers.plan_placements(plan.LLM_PARAMS, usable, cards):
            print(f"  {placement.tier}  {placement.model:24} {placement.describe()}")
        return 0
