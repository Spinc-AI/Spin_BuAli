...
| # | فایل | کار |
|---|---|---|
| ۱ | [`validation.py`](validation.py) | قابل <span dir="ltr">decode</span> بودن، خالی نبودن، مدت معتبر، و اندازه‌گیری کیفیت |
...
| `validation` | حداقل/حداکثر مدت، آستانه‌ی «سکوت مطلق»، آستانه‌ی <span dir="ltr">clipping</span> و <span dir="ltr">silence</span> |
...
## اندازه‌گیری کیفیت و تشخیص تکراری

مرحله‌ی `validation` دو کار جدا انجام می‌دهد و تفاوتشان مهم است.

**اعتبارسنجی** تصمیم می‌گیرد فایل اصلاً ارزش پردازش دارد یا نه — و اگر نداشته
باشد خطا می‌دهد. **اندازه‌گیری** فقط توصیف می‌کند که صدا چگونه است و هرگز
فایلی را رد نمی‌کند:

| ستون | معنی |
|---|---|
| `peak` / `rms` | بیشینه و توان مؤثر سیگنال |
| `clipping_ratio` | سهم نمونه‌هایی که به سقف دامنه رسیده‌اند |
| `silence_ratio` | سهم تقریبی سکوت (آستانه‌ی دامنه، نه <span dir="ltr">VAD</span>) |
| `duplicate_of` | نام اولین فایلی که همین محتوا را داشت |

`clipping_ratio` مهم‌ترینشان است. رسیدن نمونه به سقف دامنه یعنی سرِ موج بریده
شده و هیچ مرحله‌ای بعداً آن را برنمی‌گرداند — نه <span dir="ltr">resample</span>،
نه حذف نویز، نه تنظیم بهره. اگر چند درصد از یک مجموعه <span dir="ltr">clip</span>
شده باشد، مشکل از ضبط است و ارزان‌تر است میکروفون درست شود تا اینکه مدل‌ها
مدام دوباره تنظیم شوند.

`silence_ratio` عمداً تقریبی است: فقط آستانه‌ی دامنه. می‌گوید چقدر از فایل مکث
است؛ اینکه گفتار *کجاست* کار `vad.py` است.

تکراری‌ها با <span dir="ltr">SHA-256</span> پیدا می‌شوند، نه با نام فایل. دو فایل
با محتوای یکسان همان یک ضبط‌اند هرچه نامشان باشد، و چون `file_id` از همان
<span dir="ltr">digest</span> ساخته می‌شود پوشه‌ی خروجی‌شان هم یکی است. تغییر نام
نمی‌تواند آن را پنهان کند — و دقیقاً همین حالت است که با مقایسه‌ی نام از دست
می‌رود.
